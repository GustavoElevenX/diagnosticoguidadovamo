...
export async function startDiagnostic(payload = {}) {
  ...
  const intro = `Oi, ${leadName.split(' ')[0]}. Vou criar seu Mapa de Vazamento de Vendas. Minha função é identificar, em poucos minutos, onde sua operação comercial pode estar perdendo vendas antes de falar em IA, CRM ou automação.`;
...
export async function finalizeDiagnostic(payload = {}) {
  ...
  const updatePayload = {
    ...
    urgency_level: diagnostic.raw_answers?.implementation_urgency || diagnostic.raw_answers?.urgency || null,
...
export async function runDailyLearning(payload = {}, headers = {}) {
  ...
  const { data, error } = await supabase
    .from('diagnostics')
    .select('company_name, role, segment, seller_count, sales_channel, sales_model, raw_answers, score_previsibilidade, score_fit_vamo, recommended_next_step, final_report, completed_at')
    .eq('status', 'completed')
