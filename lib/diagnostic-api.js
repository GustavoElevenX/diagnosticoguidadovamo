...
function requireString(value, field) {
  if (!value || !String(value).trim()) throw new Error(`Campo obrigatorio: ${field}.`);
...
async function fetchDiagnostic(supabase, diagnosticId) {
  ...
  if (error || !data) throw new Error('Diagnostico nao encontrado.');
  return data;
}
...
export async function startDiagnostic(payload = {}) {
  ...
  if (error) throw new Error(`Erro ao iniciar diagnostico: ${error.message}`);

  const intro = `Oi, ${leadName.split(' ')[0]}. Eu sou a IA de Performance Comercial da VAMO. Vou conduzir um Raio-X rapido para encontrar vazamentos de previsibilidade, performance e dinheiro depois que o lead chega.`;
...
export async function receiveDiagnosticMessage(payload = {}) {
  ...
  if (!question) throw new Error('Etapa invalida do diagnostico.');
  ...
  if (diagnostic.status === 'completed') throw new Error('Diagnostico ja finalizado.');
  ...
  if (nextQuestion) {
    ...
    await insertMessage(supabase, diagnosticId, {
      ...
      metadata: toPublicQuestion(nextQuestion),
    });
    assistantMessages.push({ sender: 'assistant', message: nextQuestion.question, metadata: toPublicQuestion(nextQuestion) });
  } else {
  ...
  const { error } = await supabase
    ...
    .update(updates)
    .eq('id', diagnosticId);
  if (error) throw new Error(`Erro ao atualizar diagnostico: ${error.message}`);
  ...
  return {
    ...
    progress: nextQuestion ? getProgress(nextQuestion.id) : getCompletionProgress(),
  };
}
...
export async function finalizeDiagnostic(payload = {}) {
  ...
  if (diagnostic.status === 'completed' && diagnostic.final_report && Object.keys(diagnostic.final_report).length) {
    return { diagnostic_id: diagnosticId, report: diagnostic.final_report };
  }
  ...
  if (error) throw new Error(`Erro ao salvar relatorio final: ${error.message}`);
  ...
  return { diagnostic_id: diagnosticId, report };
}
...
export async function getDiagnosticReport(payload = {}) {
  ...
  const supabase = createSupabaseServerClient();
  const diagnostic = await fetchDiagnostic(supabase, diagnosticId);
  if (diagnostic.status !== 'completed') throw new Error('Diagnostico ainda nao foi finalizado.');
  return {
    diagnostic_id: diagnosticId,
    status: diagnostic.status,
    report: diagnostic.final_report,
  };
}
...
export async function runDailyLearning(payload = {}, headers = {}) {
  ...
  if (error) throw new Error(`Erro ao buscar diagnosticos: ${error.message}`);
