...
export async function startDiagnostic(payload = {}) {
  ...
  const intro = `Oi, ${leadName.split(' ')[0]}. Vou construir seu Mapa de Vazamento de Vendas. Em poucos minutos, vou entender como sua operação comercial funciona hoje, onde o processo pode estar perdendo velocidade, previsibilidade ou padrão, e qual estrutura deveria ser priorizada. IA, CRM e automação entram depois do diagnóstico, não antes.`;
...
export async function receiveDiagnosticMessage(payload = {}) {
  ...
  if (nextQuestion) {
    ...
    assistantMessages.push({
      sender: 'assistant',
      message: 'Perfeito. Vou consolidar suas respostas e gerar o Mapa de Vazamento de Vendas.',
...
export async function finalizeDiagnostic(payload = {}) {
  ...
  const updatePayload = {
    ...
    score_previsibilidade: report.score_previsibilidade_comercial || report.score_previsibilidade,
    ...
    leak_category: report.vazamentos?.[0]?.titulo || null,
  ...
  if (error && /recommended_structure|urgency_level|leak_category/i.test(error.message || '')) {
    const { recommended_structure, urgency_level, leak_category, ...legacyPayload } = updatePayload;
