...
export async function startDiagnostic(payload = {}) {
  ...
  const intro = `Oi, ${leadName.split(' ')[0]}. Eu sou a IA de Performance Comercial da VAMO. Vou conduzir um Mapa de Previsibilidade para encontrar gargalos de previsibilidade, performance e dinheiro depois que o lead chega.`;
...
export async function receiveDiagnosticMessage(payload = {}) {
  ...
  if (nextQuestion) {
    ...
    assistantMessages.push({
      sender: 'assistant',
      message: 'Perfeito. Vou consolidar suas respostas e gerar o dashboard final do Mapa de Previsibilidade.',
