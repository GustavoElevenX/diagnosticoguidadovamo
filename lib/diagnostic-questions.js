...
export const DIAGNOSTIC_BLOCKS = [
  {
    ...
    title: 'Operação',
    insight: 'Antes de falar em tecnologia ou automação, a Vértice precisa entender como a operação acontece hoje e qual é o nível de complexidade do fluxo principal.',
  },
  {
    id: 'processo_real',
    title: 'Processo real',
    insight: 'Aqui a leitura separa empresas que têm um processo operacional claro daquelas que ainda dependem de memória, improviso ou cobrança manual.',
  },
  {
    id: 'aproveitamento_oportunidades',
    title: 'Aproveitamento das oportunidades',
    insight: 'Nem toda perda vem da falta de demanda. Muitos resultados escapam porque a oportunidade não recebe velocidade, contexto ou próxima ação clara.',
  },
  {
    id: 'ponto_critico_principal',
    title: 'Ponto crítico principal',
    insight: 'Este bloco identifica onde a operação perde mais força: entrada, triagem, execução, acompanhamento, gestão, previsibilidade ou continuidade.',
  },
  {
    id: 'continuidade_comercial',
    title: 'Continuidade operacional',
    insight: 'Um resultado raramente se perde em um único ponto. Normalmente ele escapa quando não existe disciplina de próxima ação, retomada e acompanhamento.',
  },
  {
  ...
  },
  {
    ...
    insight: 'Ferramenta sem processo vira mais uma tela. Processo sem uso real vira intenção. A Vértice mede se a estrutura consegue entrar na rotina do time.',
  },
  {
  ...
  },
  {
    id: 'contato',
    title: 'Contato',
    insight: 'Com os dados principais preenchidos, a Vértice consegue salvar o mapa e enviar uma leitura personalizada com a prioridade de implantação.',
  },
];
...
export const DIAGNOSTIC_QUESTIONS = [
  {
    ...
    type: 'button',
    question: 'Quantas pessoas atuam diretamente na operação que você quer estruturar?',
    helper: 'Considere quem atende, qualifica, executa, acompanha demandas, faz propostas, entrega ou gere a rotina.',
  ...
  },
  {
    ...
    blockId: 'operacao_comercial',
    type: 'button',
    question: 'Qual modelo descreve melhor a operação da sua empresa?',
    helper: 'A ideia aqui não é classificar segmento. É entender como o fluxo principal acontece e qual nível de processo ele exige.',
    options: [
      'Operação comercial consultiva B2B',
      'Serviço local com atendimento recorrente',
      'Produto ou serviço de ticket recorrente',
      'Fluxo por proposta/orçamento',
      'Operação com triagem e repasse',
      'Operação concentrada no dono ou sócios',
      'Operação com equipe externa',
      'Outro modelo',
    ],
  ...
  },
  {
    ...
    type: 'button',
    question: 'Hoje o caminho da demanda, da entrada até a conclusão, está claro para todo o time?',
    ...
    options: [
      ...
      'Não temos um processo operacional claro',
    ],
  ...
  },
  {
    ...
    blockId: 'ponto_critico_principal',
    type: 'button',
    question: 'Em qual parte da operação você sente mais perda hoje?',
    helper: 'Escolha a opção mais próxima. A IA vai cruzar com as outras respostas para separar percepção de ponto crítico real.',
    options: [
      'Receber demandas com perfil certo',
      'Responder rápido e com contexto',
      'Triar melhor antes de passar para execução',
      'Transformar interesse em avanço concreto',
      'Retomar demandas que esfriam',
      'Enxergar onde o fluxo trava antes do fim do mês',
  ...
  },
  {
    ...
    type: 'button',
    question: 'Depois que uma demanda ou oportunidade entra, existe disciplina clara de próxima ação?',
    helper: 'Aqui a análise mede se cada item tem dono, prazo, contexto e retomada definidos.',
    options: [
  ...
  },
  {
    ...
    blockId: 'gestao_previsibilidade',
    type: 'button',
    question: 'Existe uma rotina operacional para revisar oportunidades, gargalos e ações da semana?',
    ...
    options: [
      ...
      'Não existe rotina operacional estruturada',
    ],
  ...
  },
  {
    ...
    helper: 'A Vértice precisa medir adoção porque implantação só vale quando vira rotina real, não quando vira mais uma tela esquecida.',
    options: [
  ...
  },
  {
    ...
    type: 'button',
    question: 'Se a Vértice fosse priorizar uma primeira estrutura, qual resultado faria mais sentido agora?',
    ...
    options: [
      ...
      'Mais padrão operacional entre as pessoas do time',
      'Propostas ou entregas mais rápidas e melhor acompanhamento',
  ...
  },
  {
    ...
    blockId: 'contato',
    type: 'contact',
    question: 'Já tenho dados suficientes para montar seu Mapa do Ponto Crítico da Operação Comercial. Para salvar seu resultado e a Vértice enviar uma leitura personalizada, qual WhatsApp podemos usar?',
...
function adaptQuestion(question, answers = {}) {
  ...
  if (question.id === 'business_model' && sellerCount.includes('1 a 2')) {
    return 'Com um time menor, o ponto crítico normalmente aparece na dependência do dono, na falta de rotina e na perda de contexto entre conversas. Qual modelo descreve melhor a operação da sua empresa?';
  }

  if (question.id === 'business_model' && (sellerCount.includes('11') || sellerCount.includes('21'))) {
    return 'Com esse tamanho de time, o risco costuma estar menos na falta de esforço e mais na falta de padrão, visibilidade e correção a tempo. Qual modelo descreve melhor a operação da sua empresa?';
  }

  if (question.id === 'commercial_process_clarity' && businessModel.includes('consultiva')) {
    return 'Em operação consultiva, o processo pesa muito porque o fluxo depende de contexto, próxima ação e continuidade. Hoje o caminho da demanda, da entrada até a conclusão, está claro para todo o time?';
  }

  if (question.id === 'commercial_process_clarity' && businessModel.includes('dono')) {
    return 'Quando o fluxo depende do dono ou dos sócios, o principal risco é a operação não escalar o padrão de quem vende melhor. Hoje o caminho da demanda está claro para todo o time?';
  }

  if (question.id === 'opportunity_handling' && /nao temos|cabeca|nao esta padronizado/.test(processClarity)) {
    return 'Aqui já aparece um possível ponto crítico: quando o processo não está claro, cada oportunidade depende demais de memória, improviso ou cobrança manual. Quando uma nova oportunidade aparece, o que acontece na prática?';
  }

  if (question.id === 'next_step_discipline' && /retomar|esfriam|interesse|proposta/.test(bottleneck)) {
    return 'Esse ponto costuma esconder perda operacional. O cliente até demonstra interesse, mas a operação não garante a próxima ação no tempo certo. Depois que o cliente demonstra interesse, existe disciplina clara de próxima ação?';
  }
  ...
  if (question.id === 'commercial_routine' && /nao temos|perto do fechamento|perguntar|conversas/.test(visibility)) {
    return 'Quando a visibilidade depende de perguntar ou olhar conversa por conversa, a gestão vira reação. Existe uma rotina operacional para revisar oportunidades, gargalos e ações da semana?';
  }
...
export function getAnswerInsight(questionId, answerValue) {
  ...
  if (questionId === 'seller_count') {
    ...
    if (normalized.includes('11') || normalized.includes('21')) {
      return 'Em times maiores, o ponto crítico geralmente aparece em falta de padrão, baixa visibilidade e correção tardia do gestor.';
    }
    return 'Esse tamanho de operação já permite identificar se o fluxo depende mais de pessoas específicas ou de um processo operacional repetível.';
  }

  if (questionId === 'business_model') {
    if (normalized.includes('consultiva')) {
      return 'Operação consultiva exige contexto e continuidade. Sem processo, o valor percebido se perde entre uma interação e outra.';
    }
    if (normalized.includes('proposta') || normalized.includes('orcamento')) {
      return 'Quando o fluxo depende de proposta, o ponto crítico costuma aparecer entre interesse, envio, retomada e fechamento.';
    }
    if (normalized.includes('dono')) {
      return 'Quando o dono concentra a operação, a empresa até consegue vender, mas tem dificuldade para transformar esse padrão em rotina operacional.';
    }
    return 'O modelo operacional define onde a empresa precisa de mais estrutura: entrada, triagem, execução, acompanhamento ou gestão.';
  }

  if (questionId === 'commercial_process_clarity') {
    if (/cada pessoa|nocao geral|cabeca|nao temos/.test(normalized)) {
      return 'Esse é um sinal forte de ponto crítico de processo. Antes de automatizar, é preciso definir como a operação deveria acontecer.';
    }
  ...
  if (questionId === 'opportunity_handling') {
    if (/depende|demora|contexto se perde|nao temos controle/.test(normalized)) {
      return 'Aqui pode existir perda logo no início da jornada. Oportunidade sem dono, contexto ou velocidade tende a esfriar antes de virar resultado.';
    }
  ...
  if (questionId === 'sales_bottleneck') {
    ...
    if (normalized.includes('responder rapido')) {
      return 'Velocidade sem contexto não sustenta uma operação consultiva. O ponto é responder rápido e conduzir o próximo passo corretamente.';
    }
    if (normalized.includes('retomar')) {
      return 'Esse é um dos pontos críticos mais comuns: o cliente demonstra interesse, mas a empresa não sustenta acompanhamento até a decisão.';
    }
    ...
    return 'Esse ponto vai orientar qual estrutura da Vértice deve ser priorizada no relatório final.';
  }

  if (questionId === 'next_step_discipline') {
    if (/depende|quentes|sem retomada|nao conseguimos medir/.test(normalized)) {
      return 'Esse é um ponto crítico de continuidade. A operação não termina quando o cliente demonstra interesse; ela depende de próxima ação clara.';
    }
  ...
  if (questionId === 'management_visibility') {
    if (/manual|perto do fechamento|perguntar|conversas|nao temos/.test(normalized)) {
      return 'Esse é um sinal de baixa previsibilidade. Quando o gestor só enxerga tarde, a correção chega depois que o resultado já escapou.';
    }
  ...
  if (questionId === 'commercial_routine') {
    if (/sem muita profundidade|resultado aperta|lembrar|nao existe/.test(normalized)) {
      return 'Sem rotina operacional, a gestão vira reação. A operação precisa de cadência para revisar gargalos e corrigir antes do resultado fechar.';
    }
    return 'Rotina operacional bem executada transforma dados em ação e evita que o time dependa só de cobrança pontual.';
  }

  if (questionId === 'process_technology_adoption') {
    if (/perde forca|ignora|nao virou rotina|ainda nao tentamos/.test(normalized)) {
      return 'Esse ponto é decisivo para a Vértice. Implantação só tem valor quando o processo entra na rotina real do time.';
    }
  ...
  if (questionId === 'implementation_priority') {
    return 'Perfeito. Agora a análise vai cruzar operação, ponto crítico, visibilidade e adoção para indicar a primeira frente de implantação mais coerente.';
  }
