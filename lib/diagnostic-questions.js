...
export function shouldAskQuestion(question, answers = {}) {
  ...
  const leadLoss = normalizedAnswer(answers.main_loss_area);
  ...
  if (question.id === 'lead_followup_loss') {
    return leadLoss.includes('follow-up') || leadLoss.includes('leads esquecidos') || leadLoss.includes('nao sei');
  }
