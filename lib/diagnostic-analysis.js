...
export function getOperationLevel(score) {
  ...
  if (value >= 48) return 'Operação dependente de pessoas';
  if (value >= 32) return 'Operação com pontos críticos relevantes';
  return 'Operação sem estrutura suficiente';
}
...
export function calculatePillarScores(answers = {}) {
  ...
  let clarezaProcessoComercial = optionScore(processClarity, [
    ...
    ['cabeça do dono', 34],
    ['cabeca do dono', 34],
    ['Não temos um processo operacional claro', 22],
    ['processo operacional claro', 22],
    ['Sim, com clareza', 82],
    ['Mais ou menos', 58],
  ...
  const rotinaPrevisibilidade = optionScore(routine, [
    ...
    ['lembrar e cobrar', 35],
    ['Não existe rotina operacional estruturada', 22],
    ['rotina operacional estruturada', 22],
  ], 50);
  ...
  const prioridadeImplantacao = optionScore(priority, [
    ...
    ['Mais padrão operacional entre as pessoas do time', 82],
    ['Mais padrão comercial entre as pessoas do time', 82],
    ['padrão comercial', 82],
    ['padrao comercial', 82],
    ['Propostas ou entregas mais rápidas e melhor acompanhamento', 78],
...
export function calculateFitScore(answers = {}) {
  ...
  if (/organizar o processo|visibilidade|retomada|velocidade|propostas|entregas|padrao comercial|padrao operacional/.test(combined)) score += 10;
...
export function buildLeaks(answers = {}, pillars = calculatePillarScores(answers)) {
  ...
  if (pillars.clareza_processo_comercial < 60) {
    addLeak({
      titulo: 'Processo operacional sem padrão suficiente',
      explicacao: 'A venda parece depender mais de pessoas, memória ou improviso do que de uma rotina operacional clara.',
  ...
  if (pillars.continuidade_comercial < 60) {
    addLeak({
      ...
      impacto: 'Resultados quentes podem morrer sem uma perda explícita, apenas por falta de continuidade.',
  ...
  if (pillars.rotina_previsibilidade < 60) {
    addLeak({
      titulo: 'Rotina operacional fraca ou reativa',
      ...
      impacto: 'Sem rotina, dados não viram ação e a operação depende de cobrança pontual.',
  ...
  while (leaks.length < 3) {
    addLeak([
      ...
      },
      {
        ...
        explicacao: 'Parte da execução parece depender de memória, experiência individual ou cobrança direta.',
...
export function recommendVerticeStructure(answers = {}, pillars = calculatePillarScores(answers), leaks = []) {
  ...
  if (
    ...
    return {
      nome: 'Diagnóstico Operacional Vértice',
      tipo: 'engenharia de processo operacional',
      motivo: 'Antes de implantar tecnologia ou automação, a operação precisa definir etapas, critérios, responsabilidades e primeira frente de correção.',
  ...
  if (priority.includes('velocidade') || priority.includes('qualificacao') || combined.includes('responder rapido')) {
    return {
      ...
      tipo: 'qualificação e velocidade operacional',
      motivo: 'O ponto crítico aparece no início da jornada, onde a oportunidade precisa ser atendida, qualificada e repassada com contexto.',
      proximo_passo: 'Definir critérios de qualificação, regras de passagem para o time e integração com a rotina operacional existente.',
    };
  }
  ...
  if (priority.includes('proposta') || priority.includes('fechamento')) {
    return {
      ...
      proximo_passo: 'Padronizar geração de proposta, contexto da oportunidade, alertas de abertura e retomada de fechamento.',
    };
  }

  if (priority.includes('visibilidade') || combined.includes('funil trava') || combined.includes('fluxo trava') || (pillars.visibilidade_gestao < 55 && !priority.includes('retomada') && !priority.includes('esquecidas'))) {
    return {
      nome: 'Copiloto do Gestor',
      tipo: 'gestão e maturidade operacional',
  ...
  if (combined.includes('retomar') || combined.includes('esquecidas') || combined.includes('proxima acao') || combined.includes('continuidade')) {
    return {
      ...
      tipo: 'continuidade e retomada operacional',
  ...
  if (combined.includes('padrao comercial') || combined.includes('padrao operacional') || combined.includes('padrao de execucao') || pillars.rotina_previsibilidade < 55) {
    return {
      nome: 'Camada de Inteligência do Funil',
      tipo: 'processo, dados e execução operacional',
  ...
  if (combined.includes('perfil certo') || combined.includes('atrair oportunidades') || combined.includes('receber demandas')) {
    return {
  ...
  return {
    ...
    motivo: 'As respostas indicam mais de um possível ponto crítico. Antes de implantar tecnologia, é melhor priorizar a frente com maior impacto operacional.',
    proximo_passo: 'Validar o mapa com a Vértice e escolher a primeira estrutura a implantar com base em impacto, esforço e adoção.',
  };
}
...
export function detectContradictions(answers = {}) {
  ...
  if (/perde forca|ignora|nao virou rotina/.test(adoption)) {
    contradictions.push('A empresa já tentou estruturar processo ou tecnologia, mas há sinal de baixa adoção na rotina operacional.');
  }
...
export function buildActionPlan(answers = {}, leaks = []) {
  ...
  if (leakTitles.includes('processo operacional')) {
  ...
  if (leakTitles.includes('visibilidade') || leakTitles.includes('rotina')) {
    plan.push('Criar uma rotina semanal de revisão de oportunidades paradas e gargalos operacionais.');
  }
  ...
  while (plan.length < 3) {
    const additions = [
      'Separar percepção de perda e ponto crítico real por etapa do processo operacional.',
...
function buildCentralDiagnosis(pillars) {
  if (pillars.clareza_processo_comercial < 55) {
    return 'A operação ainda depende demais de execução individual. Antes de implantar IA ou automações, a empresa precisa padronizar a jornada da oportunidade e definir onde a tecnologia entra para reduzir perda de contexto, atraso e falta de acompanhamento.';
  }
  ...
  return 'A operação já tem alguma base, mas precisa priorizar a primeira estrutura que reduz ponto crítico com processo, inteligência operacional e acompanhamento de adoção.';
}
...
export function buildFallbackReport(diagnostic = {}) {
  ...
  const recommendedStructure = recommendVerticeStructure(answers, pillarScores, leaks);
  ...
  return {
    score_maturidade_operacional: score,
    score_previsibilidade: score,
    score_resultados_previsiveis: score,
    nivel_operacao: level,
    resumo_executivo: 'Seu mapa indica uma operação com pontos críticos relevantes de processo e previsibilidade. O problema principal não parece ser apenas gerar mais oportunidades, mas transformar as oportunidades existentes em uma rotina operacional com próxima ação, visibilidade e correção durante o mês.',
    diagnostico_central: buildCentralDiagnosis(pillarScores),
    scores_pilares: pillarScores,
    pontos_criticos: leaks,
    contradicoes: contradictions,
    estrutura_recomendada: recommendedStructure,
    por_que_nao_comecar_pela_ferramenta: 'Começar pela ferramenta agora aumentaria o risco de automatizar um processo que ainda não está claro. A primeira decisão precisa ser sobre o gargalo operacional certo: o que medir, onde agir, quem usa e qual rotina sustenta a implantação.',
    recomendacao_imediata: actionPlan[0] || 'Validar o mapa e priorizar a primeira estrutura operacional antes de implantar qualquer ferramenta.',
    plano_acao: actionPlan,
    fit_vertice_score: fitScore,
    recomendacao_comercial: nextStep,
    mensagem_cta: 'A Vértice pode validar esse mapa e mostrar qual estrutura deve ser implantada primeiro para reduzir pontos críticos operacionais sem começar pela ferramenta errada.',
  };
}
...
export function normalizeReport(report, diagnostic = {}) {
  ...
  normalized.score_maturidade_operacional = clamp(
    report?.score_maturidade_operacional ??
    report?.score_previsibilidade ??
    report?.score_resultados_previsiveis ??
    fallback.score_maturidade_operacional
  );
  normalized.score_previsibilidade = normalized.score_maturidade_operacional;
  normalized.score_resultados_previsiveis = normalized.score_maturidade_operacional;
  normalized.fit_vertice_score = clamp(normalized.fit_vertice_score);
  normalized.nivel_operacao = normalized.nivel_operacao || getOperationLevel(normalized.score_maturidade_operacional);
  normalized.pontos_criticos = Array.isArray(normalized.pontos_criticos) && normalized.pontos_criticos.length ? normalized.pontos_criticos.slice(0, 3) : fallback.pontos_criticos;
  while (normalized.pontos_criticos.length < 3) {
    normalized.pontos_criticos.push(fallback.pontos_criticos[normalized.pontos_criticos.length]);
  }
  ...
  normalized.recomendacao_comercial = ['reuniao imediata', 'nutricao estrategica', 'sem fit agora'].includes(recommendation)
    ...
    : classifyCommercialRecommendation(diagnostic.raw_answers || {}, normalized.fit_vertice_score);
