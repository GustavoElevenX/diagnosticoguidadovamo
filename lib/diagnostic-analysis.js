...
export function buildLeaks(answers = {}, pillars = calculatePillarScores(answers)) {
  const candidates = [
    ...
    {
      title: 'Comissão pouco conectada ao comportamento',
      score: pillars.comissao_incentivo,
      explanation: 'A regra ou a visibilidade da comissão não orienta o vendedor durante o mês.',
      impact: 'A comissão vira apenas pagamento, não instrumento diário de performance.',
    },
    {
...
export function buildFallbackReport(diagnostic = {}) {
  ...
  return {
    ...
    resumo_executivo: `Seu Raio-X indica uma operação em nível "${level}". Os principais riscos estão ligados a previsibilidade, rotina de correção e aproveitamento das oportunidades depois que o lead chega.`,
